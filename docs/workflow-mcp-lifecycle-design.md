...
- Codex 0.145 会把 MCP namespace 工具延迟到 `tool_search` 按需发现。第三方 Chat Completions 渠道的兼容路由现按 Codex 模型协议声明 `supports_search_tool`，并把搜索结果中的 MCP namespace 工具展开为上游函数调用、再还原为带 namespace 的 Responses 调用；否则 MCP Server 即使已连接且列出 `workflow_create`，模型仍看不到可用的发现入口。
