...
- 支持本地 Claude Code / Codex / CodeBuddy 会话互相迁移：在详情页或右键菜单选择目标 Agent，应用会生成目标格式的新会话并用默认终端打开；长会话会先压缩，压缩不可用时安全截断。
...
在本地 Claude Code、Codex 和 CodeBuddy 会话的详情页或右键菜单中点击 **迁移到…**，可以把当前会话转换成另外两种 Agent 的原生会话文件，并在迁移完成后用默认终端打开目标会话。
...
迁移会尽量保留标题、项目路径、用户/助手消息和可继续工作的上下文。不同 Agent 的私有状态、工具调用内部结构、推理细节和 provider 专属元数据不会被硬翻译。长会话超过长度阈值时，会优先使用已配置的 AI 摘要 Provider 生成迁移交接摘要；如果 Provider 不可用或压缩失败，会自动降级为本地截断，并在结果中提示。若目标 CLI 或终端启动失败，目标会话文件仍会保留，界面会显示可复制的 resume 命令。

首版迁移仅支持本机会话。SSH 远程会话仍保持只读索引和远程 Resume，不会被迁移或写回远端。
