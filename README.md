...
- 支持 MCP server：通过 `agent-session-search-mcp` 以只读方式把历史会话开放给 Claude Code / Codex，提供 `search_sessions`（含摘要的全文检索）、`get_session`（支持 offset 翻页）、`list_projects`、`list_tags` 工具，可在设置里一键启用/停用。
