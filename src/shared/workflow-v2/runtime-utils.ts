...
// Shared runtime helpers that translate low-level workflow events into stable
// UI/persistence-friendly summaries without importing main-process code.
...
export function projectNodeStates(
  ...
  // Event logs are append-only; this function projects them into the latest
  // per-node status snapshot that renderer/store code can consume directly.
  ...
  return order.map((nodeId) => {
    ...
    if (status === "paused" && latest.intervention) {
      item.intervention = structuredClone(latest.intervention);
    }
...
export function workflowStoragePlanFor(workflowId: string, runId: string): WorkflowStoragePlan {
  // Workflow outputs are intentionally namespaced by workflow + run id so
  // downstream agents can write artifacts without inventing their own layout.
...
export function workflowProgressAfterFailure(
  progress: WorkflowRunProgressItem[],
  errorMessage: string,
): WorkflowRunProgressItem[] {
  // When a higher layer fails before it records a node_failed event, synthesize
  // a failed progress item so the UI does not get stuck on "running".
  ...
  return progress.map((item, itemIndex) =>
    itemIndex === targetIndex
      ? { ...item, status: "failed", detail: errorMessage }
      : item,
  );
}

export function taskArtifact(task: TaskRun): string {
  // Use the most useful user-facing artifact first, then degrade to error text
  // so higher layers always have something stable to summarize or persist.
  const assistantMessage = [...task.messages]
    .reverse()
    .find((message) => message.role === "assistant" && message.content.trim());
  if (assistantMessage) return assistantMessage.content.trim();
  const errorMessage = [...task.messages]
    .reverse()
    .find((message) => message.role === "error" && message.content.trim());
